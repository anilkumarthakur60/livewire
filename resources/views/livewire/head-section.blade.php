            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <ul class="navbar-nav navbar-nav-scroll mx-auto ">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link {{ $category->childrens_count?'dropdown-toggle':'' }}  {{ $loop->first?'active':'' }}"
                                href="#" id="homeMenu" data-bs-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">{{ $category->name }}</a>

                            @if ($category->childrens_count)


                            <ul class="dropdown-menu" aria-labelledby="homeMenu">
                                @foreach ($category->childrens as $child)
                                <li>
                                    <a class="dropdown-item active" href="index.html">{{ $child->name }}</a>
                                </li>
                                @endforeach
                            </ul>
                            @endif
                ...
                <div class="nav flex-nowrap align-items-center">
                    ...
                    <div class="nav-item d-block">
                        <a href="{{ route('dashboard') }}" class="btn btn-sm btn-danger mb-0 mx-2">Dashboard!</a>
                    ...
                    @else
                    <div class="nav-item d-block">
                        <a href="{{ route('login') }}" class="btn btn-sm btn-danger mb-0 mx-2">Loign</a>
                    </div>
                    <div class="nav-item d-block">
                        <a href="{{ route('register') }}" class="btn btn-sm btn-info mb-0 mx-2">Register</a>
                    </div>
                    @endauth
